...
def evaluate_model(video_id, folder='', gen_video=False, gt=''):

    # run model to generate predictions
    perform_video_od(video_id, gen_video, folder)


    # load model predictions along with ground_truth
    with open('ground_truth/jsons/predictions.json') as p:
...
if __name__ == '__main__':
    ...
    if args.folder == '':
        evaluate_model(args.video_id, args.folder, args.gen_video, args.gt)
    else:
                ...
                if mimestart != None:
                    ...
                    if mimestart in ['video']:
                        ...
                        try:
                            ...
                            print(filepath)
                            evaluate_model(os.path.splitext(file)[0], filepath, args.gen_video, args.gt)
