...
def parse_kinetics_annotations(input_csv, ignore_is_cc=False):
    ...
    #remove videos already in folder
    error_counter = 0
    for index, row in df.iterrows():
        ...
        print(full_vid_path)
        # if video does not exist, leave in csv to be downloaded
        if os.path.exists(full_vid_path) == False:
            continue
        # if video does exist, see if it works
        else:
            try:
    ...
    print(f"downloading {len(df)} videos")
...
if __name__ == '__main__':
    ...
    p.add_argument('-t', '--tmp-dir', type=str, default='/tmp/survai_var')
